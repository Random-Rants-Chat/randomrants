...
function getIsUserFromValue(uservalue, msgData) {
  ...
  if (uservalue.startsWith("~")) {
    ...
    if (usernameInputElement.value.toLowerCase().startsWith(lowercased.slice(1,lowercased.length))) {
      return true;
    }
  }
  ...
  notifyMessage: function (props,msg) {
    if (getIsUserFromValue(props[0],msg)) {
      ...
      try{
        ...
        notifyCommandLastNotification = new Notification(`Random Rants - Admin Commands.`, {
